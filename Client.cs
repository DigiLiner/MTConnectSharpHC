...
using RestSharp; // Nuget .NET 4.0 için uyumlu olan en üst versiyon 105.2.3.0. Bunun üstündekiler .NET4.5 istiyor // Aralık 2020
...
namespace MTConnectSharp
{
    ...
    public class MTConnectClient : IMTConnectClient, IDisposable
    {
        ...
        public void Probe()
        {
            if (_probeStarted && !ProbingCompleted)
            {
                return;
        ...
        private void ParseProbeResponse(IRestResponse response)
        {
            ...
            try
            {
                ...
                xdoc = XDocument.Load(new StringReader(response.Content));
                if (_devices.Any())
                    _devices.Clear();
            }
            catch (Exception e)
        ...
        private static List<DataItem> GetAllDataItems(IList<Component> components)
        {
        ...
        private void StreamingTimerElapsed(object sender, ElapsedEventArgs e)
        {
            ...
            {
                Probe();
                Console.WriteLine("Internal Probe");
                return;
            }
            var request = new RestRequest
            {
        ...
        private void ParseStream(IRestResponse response)
        {
            using (StringReader sr = new StringReader(response.Content))
            {
                ...
                _lastSequence = Convert.ToInt64(xdoc.Descendants().First(e =>
                    {
                        if (e.Name.LocalName == "Header") return true;
                        return false;
                    })
                ...
                var xElements = xmlDataItems as XElement[] ?? xmlDataItems.ToArray();
                if (xElements.Any())
                {
                    var dataItems = xElements.Select(e => new
                    {
                    ...
                    foreach (var item in dataItems)
                    {
                        ...
                        var sample = new DataItemSample(item.value, item.timestamp);
